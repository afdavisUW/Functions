function [] = readBosmaData(name, samplingFreq, mooringBool, Paths)
...
%     cell datasets
% 
% NOTES attentuation factor still needs to be multiplied in a later
% function
...
load(strcat(Paths.dataDir, filesep, 'sensorCal.mat')); % loads the structure of slopes and intercepts for the load cell data
loadCellData = csvread([Paths.dataDir, filesep, name.sensorFile],1,0); % starts reading at second row, first column
...
motionData = tdfread([Paths.dataDir, filesep, name.motionFile],'\t'); % residuals and Rot[#]
...
tempOWCHeaveVel = diff(Data.motion.OWC.heave)/dt;
Data.motion.OWC.heaveVel = smooth(Data.motion.time(1:length(tempOWCHeaveVel)),tempOWCHeaveVel,span,'rloess');

%% incident wave data from a text file
fileID = fopen([Paths.dataDir, filesep,name.waveFile]);
formatSpec = '%s';
...
Data.wave.incidentOWC = detrend(waveDataFull{2});
...
tempVel_fps = diff(Data.wave.incidentFPS)/dt;
Data.wave.incidentFPSVel = smooth(Data.wave.time(1:length(tempVel_fps)),tempVel_fps,span,'rloess');
tempVel_owc = diff(Data.wave.incidentOWC)/dt;
Data.wave.incidentOWCVel = smooth(Data.wave.time(1:length(tempVel_owc)),tempVel_owc,span,'rloess');
% Data.wave.incidentFPSVel = diff(Data.wave.incidentFPS)/dt;

tempAcc_fps = diff(Data.wave.incidentFPSVel)/dt;
Data.wave.incidentFPSAcc = smooth(Data.wave.time(1:length(tempAcc_fps)),tempAcc_fps,span,'rloess');
tempAcc_owc = diff(Data.wave.incidentOWCVel)/dt;
Data.wave.incidentOWCAcc = smooth(Data.wave.time(1:length(tempAcc_owc)),tempAcc_owc,span,'rloess');
...
% end
save(strcat('BosmaData',char(name.runNo),'.mat'),'Data')
movefile(strcat('BosmaData',char(name.runNo),'.mat'),Paths.dataDir);
